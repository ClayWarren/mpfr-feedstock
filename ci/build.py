...
local = Path("C:/mpfr-local")
conda = [sys.executable, "C:/mpfr-tools/Scripts/conda-script.py"]
...
run(conda + ["build", "recipe", "-m", f".ci_support/{subdir.replace('-', '_')}_.yaml",
             ...
             "--croot", "C:/mpfr-build", "--output-folder", str(local), "--no-anaconda-upload",
             "--override-channels", "-c", "file:///C:/mpfr-local", "-c", "conda-forge"])
print(f"PASS: mpfr build and installed consumer tests on {subdir}", flush=True)
